...
public abstract class BaseService <T, ID> implements IBaseService<T, ID> {
    ...
    @Override
    public Mono<T> create(T o) {return getRepository().save(o);}
    @Override
    public Mono<T> findById(ID id) {
        return getRepository().findById(id);
    }
    @Override
    public Mono<T> update(T o) {return getRepository().save(o);}
    @Override
    public Mono<Void> delete(ID id) {return getRepository().deleteById(id);}
}
